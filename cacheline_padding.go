...
func testCacheLine() {
	...
	go func() { // func1
		...
		for i := int64(0); i < runTimes; i++ {
			atomic.AddUint64(&obj.num, 1)
		}
	}()
	go func() { // func2
		defer wg.Done()
		for i := int64(0); i < runTimes; i++ {
			atomic.AddUint64(&obj.num2, 1)
		}
	}()
	wg.Wait()
	println(fmt.Sprintf("runTime1 = %+v", time.Now().UnixNano()-runTime1))
	runTime2 := time.Now().UnixNano()
	wg.Add(2)
	go func() { // func1
		defer wg.Done()
		for i := int64(0); i < runTimes; i++ {
			atomic.AddUint64(&obj2.num, 1)
		}
	}()
	go func() { // func2
		defer wg.Done()
		for i := int64(0); i < runTimes; i++ {
			atomic.AddUint64(&obj2.num2, 1)
		}
	}()
	wg.Wait()
	println(fmt.Sprintf("runTime2 = %+v", time.Now().UnixNano()-runTime2))
}
